<x-backend-layout>
    @section('title', 'Paid Commissions')
    <x-slot name="styles">
    ...
    <x-slot name="breadcrumb_items">
        ...
        <li class="breadcrumb-item active">Paid Commissions</li>
    </x-slot>
    <div>
        <div class="row">
            <div class="col-sm-12">
                            ...
                            <table class="table table-striped table-bordered dt-responsive nowrap display"
                                ...
                                <tbody>
                                    @php
                                        ...
                                        $totalEarnings = 0;
                                    @endphp
                                    @foreach ($allCommissionPayouts as $key => $allCommissionPayout)
                                        <tr id="commission-record-{{ $allCommissionPayout->service->id }}">

                                            <td>{{ $allCommissionPayout->service->id }}</td>
                                            <td>{{ $allCommissionPayout->service->user->name }}</td>
                                            <td>{{ $allCommissionPayout->service->title }}</td>
                                            <td>{{ $allCommissionPayout->service->category->name }}</td>
                                            <td>{{ $allCommissionPayout->service->bankDetail->account_holder }}</td>
                                            <td>{{ $allCommissionPayout->service->bankDetail->account_number }}</td>
                                            <td>{{ $allCommissionPayout->service->bankDetail->bank->bank_name ?? '' }}
                                            </td>
                                            <td>{{ $allCommissionPayout->service->bankDetail->branch->branch_name ?? '' }}
                                            </td>
                                            <td> {{ $allCommissionPayout->service->inquiries()->where('member_status', 'completed')->whereDate('updated_at', $date)->count() }}
                                            </td>
                                            <td>{{ $allCommissionPayout->service->category->commission }}%</td>
                                            <td>
                                                @php
                                                    $totalServiceCommissions = 0;
                                                    $totalServiceEarnings = 0;
                                                @endphp
                                                @foreach ($allCommissionPayout->service->inquiries as $inquiry)
                                                    @if ($inquiry->member_status === 'completed'&& $inquiry->updated_at->toDateString() === $date)
                                                        @php
                                                            $commission = ($inquiry->cost * $allCommissionPayout->service->category->commission) / 100;
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('admin.commission.inquiries', ['service' => $allCommissionPayout->service, 'date' => $date]) }}"><b>
            ...
            $(function() {
                $("#datepicker").datepicker({
                    ...
                    onSelect: function(dateText, inst) {
                        window.location.href = "{{ route('admin.commission.paid') }}?date=" + dateText;
                    }
                });
            });
        </script>
