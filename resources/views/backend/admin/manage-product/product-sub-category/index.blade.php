<x-backend-layout>
    ...
    <x-slot name="breadcrumb_items">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.manageProduct') }}">Manage Product Category</a></li>
        <li class="breadcrumb-item active">Manage Product Subcategory</li>
    </x-slot>
    <div>
        <div class="row">
            <div class="max-w-7xl mx-auto py-5 sm:px-6 lg:px-8">
                <livewire:admin.manage-product.product-sub-category.create :productcategory="$productcategory" />
            </div>
            <div class="col-sm-12">
                <div class="card shadow">
                    <div class="card-header">
                        <h5>Manage Product Subcategory</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered dt-responsive nowrap" id="subcategory"
                                style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($subcategories as $key => $subcategory)
                                        <tr id="subcategory-record-{{ $subcategory->id }}">
                                            <td>{{ $subcategory->id }}</td>
                                            <td>{{ $subcategory->name }}</td>
                                            <td class="text-center">
                                                <a class="btn btn-sm btn-primary"
                                                    href="{{ route('admin.productSubCategory.edit', ['subcategory' => $subcategory]) }}">
                                                    <i class="fa fa-pencil"> </i>
                                                </a>
                                                <a class="btn btn-sm delete-subcategory btn-danger"
                                                    data-subcategory="{{ $subcategory->id }}"
                                                    id="subcategory-{{ $subcategory->id }}" href="javascript:void(0)">
    ...
    <x-slot name="scripts">
        ...
        <script src="{{ asset('js/admin/manage-product-category/product-sub-category.js') }}"></script>
    </x-slot>
</x-backend-layout>
