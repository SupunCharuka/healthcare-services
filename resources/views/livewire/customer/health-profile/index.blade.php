    ...
    <form wire:submit.prevent="store" class="theme-form" enctype="multipart/form-data">
        ...
        <div class="px-4 py-3 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
            <div class="card-header bg-white px-1 py-1">
                <h5 class="py-1 font-weight-bold">Add Your Medical Report</h5>
            </div>
            ...
            <div class="col-span-6 sm:col-span-4 mt-3 form-group mb-2 mt-4">
                <label class="block font-medium text-sm text-gray-700" for="document" id="bank-upload-image">Upload
                ...
                <div x-data="{ isUploading: false, progress: 0 }" x-on:livewire-upload-start="isUploading = true"
                    ...
                    <input id="old_file" type="hidden" value="{{ $health->file }}">
                    <!-- Progress Bar -->
                ...
                @if (!$errors->has('document'))
                    ...
                    @elseif($health->file)
                        @php
                            ...
                            $explodeImage = explode('.', $health->file);
                            $extension = end($explodeImage);
                        @endphp
                        @if (in_array($extension, $imageExtensions, true))
                            <img class="img-thumbnail mt-2"
                                src="{{ storage('uploads/customer/health-profile/' . $health->file) }}"
                        ...
                        @else
                            ...
                            <a href="{{ storage('uploads/customer/health-profile/' . $health->file) }}" target="blank">
                                {{ $health->file }}
                            </a>
                        @endif
